 ...
 * make scenario_test
 *
 * I've tried to document the code fairly well. For more information about the
...
namespace PVTP {
	...
	namespace Planner {
		 ...
		 */
		void MergeGoal( std::vector<PVT_S*>& Goal, std::vector<PVT_S*>& S_goal, const Constraints& c );
		...
		/**
		 * This procedure generates the upper-bounding representative trajectory
		 * by returning either UB directly, or recursively tracing out the
		 * upper boundary of the feasible region between s_i and p when UB is
		 * in collision. The resulting trajectory is stored in T, which is
		 * either a sequence of states, or empty.
		 */
		...
		/**
		 * Connect from a given state `s' on `edge' to the goal.
		 */
		...
		/**
		 * Given a starting point `p', a set of valid initial velocities V_i,
		 * and an obstacle edge `edge', connect to `edge' from `s_i' in a
		 * time-minimal way, or return an empty trajectory if no connection is
		 * possible.
		 */
		...
		/**
		 * Given a starting state `s_i', the edge `s_i' belongs to
		 * (`edge_from'), and the target edge (`edge_to'), connect to the target
		 * edge from `s_i' on `edge_from' in a time-minimal way, or return an
		 * empty trajectory if no connection is possible.
		 *
		 * NOTE: This procedure only considers edges that form convex
		 * boundaries.
		 */
		...
		/**
		 * Given a starting edge `edge' and a terminal point `p', connect to `p'
		 * in a time-minimal way, or return an empty trajectory if no connection
		 * is possible.
		 */
		...
		/**
		 * Construct a collision bounded trajectory between two states.
		 *
		 * The trajectory is stored in T.
		 */
